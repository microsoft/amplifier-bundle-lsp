...
class ProxyLspServer:
    """LSP server connection via TCP proxy.

    Connects to a persistent proxy that owns the actual LSP server subprocess.
    Same interface as LspServer — operations.py doesn't know the difference.

    Key difference from LspServer:
    - shutdown() just closes the TCP connection, does NOT kill the server
    - The proxy manages server lifecycle independently
    """
    ...
    async def connect(
        ...
        """Connect to an existing proxy and perform LSP initialization."""
        reader, writer = await asyncio.open_connection("127.0.0.1", port)
        server = cls(language, workspace, reader, writer, port, timeout=timeout)
        server._reader_task = asyncio.create_task(server._read_responses())
        await server._initialize(init_options)
        return server
    ...
    async def _read_responses(self):
        """Background task to read responses from proxy."""
        while True:
            try:
                # Read header
    ...
    async def shutdown(self):
        """Disconnect from the proxy. Does NOT kill the server.

        The proxy manages server lifecycle independently.
        We send LSP shutdown/exit to cleanly close our session with the proxy,
        but the proxy intercepts these and keeps the server alive.
        """
        try:
            await self.request("shutdown", {})
        except Exception:
